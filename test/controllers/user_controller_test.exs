defmodule Pxblog.UserControllerTest do
  ...
  @valid_create_attrs %{email: "[email protected]", username: "testuser", password: "test1234", password_confirmation: "test1234"}
  ...
  setup do
    ...
    {:ok, nonadmin_user} = TestHelper.create_user(
      user_role, %{username: "nonadmin", email: "nonadmin@test.com",
                   password: "test", password_confirmation: "test"}
    )
    {:ok, admin_role} = TestHelper.create_role(%{name: "admin", admin: true})
    {:ok, admin_user} = TestHelper.create_user(
      admin_role, %{username: "admin", email: "admin@test.com",
                    password: "test", password_confirmation: "test"}
    )
    {:ok, conn: build_conn(), user_role: user_role,
      nonadmin_user: nonadmin_user, admin_role: admin_role,
  ...
  @tag admin: true
  test "renders form for new resources when admin",
  ...
  @tag admin: true
  test "renders form for new resources when not admin",
  ...
  @tag admin: true
  test "renders form for editing choosen resource when logged in as admin",
  ...
  @tag admin: true
  test "redirects away from editing when logged in as different user",
       %{conn: conn, nonadmin_user: nonadmin_user, admin_user: admin_user} do
    conn = login_user(conn, nonadmin_user)
  ...
  test "updates chosen resource and redirects when data is valid when logged in as that user",
       %{conn: conn, nonadmin_user: nonadmin_user} do
    conn = login_user(conn, nonadmin_user)
    conn = put conn, user_path(conn, :update, nonadmin_user),
  ...
  @tag admin: true
  test "updates chosen resource and redirects when data is valid and logged in as admin",
       %{conn: conn, admin_user: admin_user} do
    conn = login_user(conn, admin_user)
